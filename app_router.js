...
const max_download_items = 20; // max number of items handled by download task per polling interval
        ...
        if (buffer && (buffer.length > 256)) {
            ...
            if (buffer) {
                try {
                    ...
                    logger.warn(`Get error message when downloading: ${msg_type}-${msg_id}:`, obj);
                } catch (err) {
